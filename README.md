...
東京・調布の喫茶＆ちょい呑み店「Saint Malo（サンマロー）」の公式サイト。

> **📌 ブランチ運用メモ（2026-06-13）**
> - `main`：暫定公開版の**シンプル店舗情報ページ**（`index.html` 1枚で完結・確定情報のみ）。Cloudflareで公開中。
> - `lp-draft`：**フルLP**（下記の縦型ランディングページ）。メニュー価格・写真・緯度経度を仕上げてから `main` に統合・公開予定。
>
> **以下のドキュメントは `lp-draft` のフルLP仕様です。**
